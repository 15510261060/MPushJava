...
public final class TcpConnection implements Connection {
    ...
    public TcpConnection(MPushClient client, PacketReceiver receiver,ClientConfig config) {
        this.client = client;
        this.logger = config.getLogger();
        this.listener = config.getClientListener();
        this.config = config;
        this.allotClient = new AllotClient();
        this.reader = new AsyncPacketReader(this, receiver, config);
    ...
    @Override
    public void onBindTokenFailed() {
        tokenFailed = true;
        listener.onBindTokenFailed(client);
    }
    ...
    private boolean doReconnect() {
        ...
        if (reconnectCount > MAX_RESTART_COUNT) {    // 超过此值 sleep 10sec
            if (connLock.await(SECONDS.toSeconds(10))) {
                state.set(State.disconnected);
                return true;
            }
            reconnectCount = 0;
