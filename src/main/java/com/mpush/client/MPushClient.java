...
/*package*/final class MPushClient implements Client, AckCallback {
    ...
    /*package*/ MPushClient(ClientConfig config) {
        ...
        this.connection = new TcpConnection(this, receiver, config);
    ...
    @Override
    public void bindUser(final String userId, final String tags, final String token) {
        if (Strings.isBlank(userId)) {
            logger.w("bind user is null");
            return;
        }
        if (connection.isTokenFailed()){
            logger.w("token is failed");
            return;
        }
        SessionContext context = connection.getSessionContext();
        ...
        context.setBindUser(userId).setTags(tags).setToken(token);
        config.setUserId(userId).setTags(tags).setToken(token);
